...
export function buildTicketHTML(
  data: TicketData,
  opts?: { ticketAncho?: string; forPrint?: boolean; showTax?: boolean },
): string {
  const {
    empresa,
    folio,
  ...
  const ahorroCliente = lineas.reduce((sum, l) => {
    ...
    const ahorroLinea = sugerido * l.cantidad - l.total;
    console.log({
      sugerido,
      cantidad: l.cantidad,
      total: l.total,
    ...
    return sum + Math.max(0, ahorroLinea);
  }, 0);
  ...
  if (empresa.logo_url) {
    hLines.push(
      `<div style="margin-bottom:6px"><img src="${empresa.logo_url}" alt="${empresa.nombre}" style="max-width:160px;max-height:80px;margin:0 auto;display:block" crossorigin="anonymous" /></div>`,
    );
  }
  hLines.push(
    `<div style="font-weight:700;font-size:22px">${empresa.nombre.toUpperCase()}</div>`,
  );
  if (empresa.razon_social)
    hLines.push(`<div style="font-size:14px">${empresa.razon_social}</div>`);
  if (empresa.rfc)
    hLines.push(`<div style="font-size:14px">RFC: ${empresa.rfc}</div>`);
  ...
  if (empresa.telefono)
    hLines.push(`<div style="font-size:13px">Tel: ${empresa.telefono}</div>`);
  if (empresa.email)
    hLines.push(`<div style="font-size:13px">${empresa.email}</div>`);
  ...
  const pagoLabel =
    condicionPago === "credito"
      ...
      : condicionPago === "contado"
        ...
        : "P/definir";
  add(
    `Pago: ${pagoLabel}${condicionPago !== "credito" && metodoPago ? ` (${metodoPago})` : ""}`,
  );
  add(div);
  ...
  for (const l of lineas) {
    const lineAmt = showTax
      ? l.total
      : l.total - (l.iva_monto ?? 0) - (l.ieps_monto ?? 0);
    ...
    if (showTax && (l.iva_monto ?? 0) > 0)
      detParts.push(`IVA${fmt(l.iva_monto!)}`);
    if ((l.precio_sugerido_publico ?? 0) > 0)
      detParts.push(`Sug ${fmt(l.precio_sugerido_publico!)}`);
    add(detParts.join(" ").substring(0, COLS));
    // Per-product promotions
    const linePromos = (promociones ?? []).filter(
      (p) => p.producto_id && p.producto_id === l.producto_id,
    );
  ...
  if (ahorroCliente > 0 && empresa.ticket_campos?.ahorro_cliente !== false) {
    add(pad("Ahorraste", fmt(ahorroCliente)));
  }
...

  {
    ...
    if (pagoAplicado != null && pagoAplicado > 0)
      add(pad("Pago", `-${fmt(pagoAplicado)}`));
  ...
  if (pagos && pagos.length > 0) {
    ...
    for (const p of pagos) {
      ...
      const label =
        fechaPart + p.metodo + (p.referencia ? ` (${p.referencia})` : "");
