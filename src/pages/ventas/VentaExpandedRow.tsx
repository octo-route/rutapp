...
export function VentaExpandedRow({
  venta,
  fmt,
  canDelete,
  onDeleteTarget,
    ...
    async function load() {
      const [lRes, pRes] = await Promise.all([
        supabase
          .from("venta_lineas")
          .select(
            "id, cantidad, precio_unitario, descuento_pct, subtotal, iva_monto, ieps_monto, total, producto_id, lista_precio_id, precio_manual, productos(nombre, unidad_granel), lista_precios(nombre, es_principal)",
          )
  ...
  const clienteNombre =
    venta.clientes?.nombre || (venta.cliente_id ? "—" : "Público en general");
  const eId = empresaId || venta.empresa_id;

  const handlePdf = async () => {
    setGeneratingPdf(true);
    try {
      const { blob, fileName, caption } = await generateVentaPdfById(
        venta.id,
        eId,
      );
      setPdfBlob(blob);
          ...
          <div className="bg-card border-b border-border px-4 py-3 space-y-3 animate-in slide-in-from-top-1 duration-200">
            ...
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-3 flex-wrap">
                ...
                <span className="text-muted-foreground text-xs">
                  {fmtDateTime(venta.created_at)}
                </span>
                <span className="text-muted-foreground text-xs">•</span>
                <span className="text-muted-foreground text-xs">
                  {CONDICION_LABELS[venta.condicion_pago] ||
              ...
              <div className="flex items-center gap-1.5 shrink-0">
                ...
                <Button
                  ...
                  disabled={printingTicket || loading}
                >
                  {printingTicket ? (
                    <Loader2 className="h-3 w-3 animate-spin" />
                  ) : (
                ...
                {venta.status !== "borrador" &&
                  (venta.saldo_pendiente ?? 0) > 0 && (
                    <Button
                      ...
                      variant="outline"
                      className="h-7 text-xs gap-1.5"
                      onClick={() => navigate("/ventas/cobranza")}
                    >
            ...
            {loading ? (
              ...
              <div className="space-y-4">
                {/* Líneas */}
                <div>
                  <h4 className="text-[11px] font-semibold text-muted-foreground uppercase mb-1">
                    Productos
                  </h4>
                  <table className="w-full text-[12px]">
                    <thead>
                      <tr className="border-b border-border text-muted-foreground">
                        ...
                        <th className="text-right py-1 font-medium w-14">
                          Cant
                        </th>
                        <th className="text-center py-1 font-medium w-10">
                          Ud
                        </th>
                        <th className="text-right py-1 font-medium w-16">
                          Monto
                    ...
                    <tbody>
                      {lineas.map((l: any) => {
                        const descMonto =
                          (l.subtotal ?? 0) * ((l.descuento_pct ?? 0) / 100);
                        const lp = (l as any).lista_precios;
                        const listaLabel = l.precio_manual
                          ? "Manual"
                          : (lp?.nombre ?? "—");
                        return (
                          <tr key={l.id} className="border-b border-border/40">
                            <td className="py-1.5">
                              {(l.productos as any)?.nombre ?? "—"}
                            </td>
                            ...
                            <td className="text-center py-1.5 text-muted-foreground">
                              {(l.productos as any)?.unidad_granel || "Pzs"}
                            </td>
                            <td className="text-right py-1.5 tabular-nums">
                              {fmt(l.subtotal)}
                            </td>
                  ...
                  <div className="flex justify-end mt-2">
                    <div className="text-[12px] space-y-0.5 min-w-[200px]">
                      ...
                      <div className="flex justify-between font-bold border-t border-border pt-0.5">
                        <span>Total</span>
        ...
        <tr className="hidden">
          <td>
            <DocumentPreviewModal
              ...
              empresaId={eId}
              defaultPhone={
                clientesList?.find((c) => c.id === venta.cliente_id)?.telefono
              }
              caption={pdfCaption}
              tipo="venta"
