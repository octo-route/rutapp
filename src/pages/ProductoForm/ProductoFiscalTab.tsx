  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 gap-x-10">
      ...
      <div>
        <OdooField
          label="IVA %"
          value={form.iva_pct ?? 16}
          type="number"
          teal
          onChange={(v) => {
            const rate = Number(v) || 0;
            set("iva_pct", rate);
            set("tiene_iva", rate > 0);
          }}
          format={(v) => `${v ?? 16}%`}
        />
        <div className="ml-[140px] -mt-1 mb-2 flex gap-2">
          {[0, 8, 16].map((rate) => (
            <button
              ...
              className={`text-[11px] px-2 py-0.5 rounded border transition-colors ${form.iva_pct === rate ? "bg-primary text-primary-foreground border-primary" : "border-border text-muted-foreground hover:border-primary/50"}`}
            >
              {rate}%
            </button>
          ))}
        ...
        <div className="odoo-field-row">
          ...
          <label className="flex items-center gap-2 cursor-pointer pt-[2px]">
            <input
              type="checkbox"
              checked={!!form.costo_incluye_impuestos}
              onChange={(e) => set("costo_incluye_impuestos", e.target.checked)}
        ...
        {form.costo_incluye_impuestos && (form.costo ?? 0) > 0 && (
          <div className="ml-[140px] text-xs text-muted-foreground bg-secondary/50 rounded p-2 mb-2">
            {(() => {
              const t = calcTax({
                precio: form.costo ?? 0,
                iva_pct: form.iva_pct ?? 16,
                ieps_pct: form.ieps_pct ?? 0,
                ieps_tipo: (form.ieps_tipo as any) || "porcentaje",
              ...
              return (
                <>
                  ...
                  <strong>
                    {s}{" "}
                    {t.precio_neto.toLocaleString("es-MX", {
                      minimumFractionDigits: 2,
                      maximumFractionDigits: 2,
                    })}
                  </strong>{" "}
                  + IEPS: {s}{" "}
                  {t.ieps_monto.toLocaleString("es-MX", {
                    minimumFractionDigits: 2,
                    maximumFractionDigits: 2,
                  })}{" "}
                  + IVA: {s}{" "}
                  {t.iva_monto.toLocaleString("es-MX", {
                    minimumFractionDigits: 2,
                    maximumFractionDigits: 2,
                  })}
                </>
        ...
        <div className="mt-2 bg-accent/30 border border-accent/50 rounded px-3 py-2 text-[11px] text-muted-foreground">
          💡 El IVA se calcula sobre el precio + IEPS (estándar fiscal
          mexicano). IEPS puede ser porcentaje o cuota fija por unidad.
        </div>
      </div>
    </div>
  );
