  ...
  return useQuery({
    ...
    queryFn: async () => {
      ...
      let q = supabase
        .from("ventas")
        .select(
          "*, clientes(nombre), vendedores:profiles!vendedor_id(nombre), venta_lineas(cantidad, precio_unitario, subtotal, total, productos(id, codigo, nombre), unidades(abreviatura))",
        )
        .eq("empresa_id", empresa!.id)
        .eq("tipo", "pedido")
        .in("status", ["confirmado", "entregado", "facturado"])
        .or(
          `and(fecha_entrega.gte.${dStr},fecha_entrega.lte.${hStr}),and(fecha_entrega.is.null,fecha.gte.${dStr},fecha.lte.${hStr})`,
        )
...
export default function ReporteEntregasPage() {
  ...
  const { data: entregas, isLoading } = useReporteEntregas(
    vendedorId,
    fechaDesde,
    fechaHasta,
  );
  ...
  const productSummary = useMemo(() => {
    ...
    const map = new Map<
      string,
      { codigo: string; nombre: string; cantidad: number; total: number }
    >();
  ...
  const byRoute = useMemo(() => {
    ...
    const map = new Map<
      string,
      { vendedor: string; entregas: typeof entregas; total: number }
    >();
    for (const e of entregas) {
  ...
  const byDate = useMemo(() => {
    ...
    const map = new Map<
      string,
      { fecha: string; entregas: typeof entregas; total: number }
    >();
    for (const e of entregas) {
  ...
  const totalMonto =
    entregas?.reduce((s, e) => s + ((e as any).total ?? 0), 0) ?? 0;
    ...
    <div className="p-4 space-y-4 min-h-full">
      ...
      <div className="hidden print:block mb-4">
        <h1 className="text-lg font-bold">
          {empresa?.nombre} — Reporte de entregas
        </h1>
        <p className="text-sm text-muted-foreground">
          {format(fechaDesde, "dd/MM/yyyy")} al{" "}
          {format(fechaHasta, "dd/MM/yyyy")}
          {vendedorId !== "todos" &&
            ` — Ruta: ${vendedores?.find((v) => v.id === vendedorId)?.nombre}`}
        </p>
      </div>
      ...
      <div className="bg-card border border-border rounded-lg overflow-hidden">
        ...
        <Table>
          ...
          <TableBody>
            {productSummary.map((p) => (
              <TableRow key={p.codigo}>
                ...
                <TableCell className="text-[12px] font-medium py-1.5">
                  {p.nombre}
                </TableCell>
                <TableCell className="text-right text-[12px] font-bold py-1.5">
                  {p.cantidad}
                </TableCell>
                <TableCell className="text-right text-[12px] py-1.5">
                  {fmt(p.total)}
                </TableCell>
              </TableRow>
            ))}
          ...
          <TableFooter>
            <TableRow>
              <TableCell colSpan={2} className="text-[11px] font-semibold">
                Total
              </TableCell>
            ...
            <Table>
              ...
              <TableBody>
                {group.entregas.map((e: any) => (
                  <TableRow key={e.id}>
                    <TableCell className="font-mono text-[11px] font-bold py-1.5">
                      {e.folio}
                    </TableCell>
                    ...
                    <TableCell className="text-center py-1.5">
                      <Badge
                        variant={
                          e.status === "entregado" || e.status === "facturado"
                            ? "default"
                            : "outline"
                        }
                        className={cn(
                    ...
                    <TableCell className="text-center text-[11px] text-muted-foreground py-1.5">
                      {e.venta_lineas?.length ?? 0}
                    </TableCell>
                    <TableCell className="text-right text-[11px] font-medium py-1.5">
                      {fmt(e.total ?? 0)}
                    </TableCell>
                  </TableRow>
                ))}
            ...
            <TableBody>
              {byRoute.map((r) => (
                <TableRow key={r.vendedor}>
                  <TableCell className="text-[12px] font-semibold py-1.5">
                    {r.vendedor}
                  </TableCell>
                  <TableCell className="text-center text-[12px] py-1.5">
                    {r.entregas.length}
                  </TableCell>
                  <TableCell className="text-center text-[12px] text-warning font-bold py-1.5">
                    {r.entregas.filter((e) => e.status === "confirmado").length}
                  </TableCell>
                  <TableCell className="text-center text-[12px] text-success font-bold py-1.5">
                    {r.entregas.filter((e) => e.status === "entregado").length}
                  </TableCell>
                  <TableCell className="text-right text-[12px] font-bold py-1.5">
                    {fmt(r.total)}
                  </TableCell>
