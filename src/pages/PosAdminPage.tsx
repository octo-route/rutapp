    ...
    <div className="space-y-4 p-4 md:p-6 max-w-7xl mx-auto">
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-muted-foreground">
              Turnos, cortes, depósitos, retiros, gastos y ventas POS.
            </p>
          </div>
        </div>
        <Button asChild size="sm" className="gap-1.5">
          <Link to="/pos">
            <ExternalLink className="h-4 w-4" /> Abrir caja POS
          </Link>
        </Button>
      </div>
      ...
      <Tabs value={tab} onValueChange={(v) => setParams({ tab: v })}>
        <TabsList className="flex-wrap h-auto">
          ...
          <TabsTrigger value="cortes" className="gap-1.5">
            <ClipboardList className="h-4 w-4" /> Cortes
          </TabsTrigger>
          <TabsTrigger value="depositos" className="gap-1.5">
            <ArrowDownCircle className="h-4 w-4" /> Depósitos
          </TabsTrigger>
          <TabsTrigger value="retiros" className="gap-1.5">
            <ArrowUpCircle className="h-4 w-4" /> Retiros
          </TabsTrigger>
          <TabsTrigger value="gastos" className="gap-1.5">
            <Receipt className="h-4 w-4" /> Gastos
          </TabsTrigger>
          <TabsTrigger value="ventas" className="gap-1.5">
            <ShoppingCart className="h-4 w-4" /> Ventas POS
          </TabsTrigger>
        </TabsList>
        <TabsContent value="turnos" className="mt-4">
          <TurnosPanel empresaId={empresaId} onView={setOpenTurno} />
        </TabsContent>
        <TabsContent value="cortes" className="mt-4">
          <CortesPanel empresaId={empresaId} onView={setOpenTurno} />
        </TabsContent>
        <TabsContent value="depositos" className="mt-4">
          <MovimientosPanel empresaId={empresaId} tipo="deposito" />
        </TabsContent>
        <TabsContent value="retiros" className="mt-4">
          <MovimientosPanel empresaId={empresaId} tipo="retiro" />
        </TabsContent>
        <TabsContent value="gastos" className="mt-4">
          <MovimientosPanel empresaId={empresaId} tipo="gasto" />
        </TabsContent>
        <TabsContent value="ventas" className="mt-4">
          <VentasPosPanel empresaId={empresaId} />
        </TabsContent>
      </Tabs>
...
function KpisRow({ empresaId }: { empresaId: string }) {
  const q = useQuery({
    ...
    queryFn: async () => {
      ...
      const abiertos = (turnos.data ?? []).filter(
        (t: any) => t.status === "abierto",
      ).length;
      const cerrados = (turnos.data ?? []).filter(
        (t: any) => t.status === "cerrado",
      ).length;
      const totalDif = (turnos.data ?? []).reduce(
        (s: number, t: any) => s + (Number(t.diferencia) || 0),
        0,
      );
      const m = (movs.data ?? []).reduce(
        (acc: any, r: any) => {
  ...
  return (
    <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-7 gap-2">
      ...
      <Kpi
        ...
        label={`Ventas POS (${d?.nVentas ?? 0})`}
        value={d?.totalVentas ?? 0}
        tone="primary"
      />
      <Kpi
      ...
      <Kpi
        ...
        label="Diferencia"
        value={d?.totalDif ?? 0}
        tone={Number(d?.totalDif ?? 0) === 0 ? "primary" : "destructive"}
      />
    </div>
  );
}
...
async function fetchTurnos(empresaId: string, soloCerrados = false) {
  ...
  const cajeroIds = Array.from(
    new Set((turnos ?? []).map((t: any) => t.cajero_id).filter(Boolean)),
  );
...
function TurnosPanel({
  ...
  const { fmt: _fmt } = useCurrency();
  const q = useQuery({
    queryKey: ["pos-admin-turnos", empresaId],
    queryFn: () => fetchTurnos(empresaId),
  });
  const turnos = q.data ?? [];
  if (q.isLoading)
    return (
          ...
          {turnos.map((t: any) => (
            ...
            >
              ...
              <td className="px-3 py-2 text-xs tabular-nums">
                {fmtDate(t.abierto_at)}
              </td>
              <td className="px-3 py-2 text-xs tabular-nums">
                {t.cerrado_at ? fmtDate(t.cerrado_at) : "—"}
              </td>
              <td className="px-3 py-2 text-right tabular-nums">
                {_fmt(t.fondo_inicial)}
              </td>
              <td className="px-3 py-2 text-right tabular-nums">
                {t.total_efectivo_esperado != null
                  ? _fmt(t.total_efectivo_esperado)
                  : "—"}
              </td>
              <td
                className={`px-3 py-2 text-right tabular-nums font-semibold ${t.diferencia == null ? "" : Number(t.diferencia) === 0 ? "text-success" : Number(t.diferencia) < 0 ? "text-destructive" : "text-warning"}`}
              >
...
function CortesPanel({
  ...
  const { fmt: _fmt } = useCurrency();
  const q = useQuery({
    queryKey: ["pos-admin-cortes", empresaId],
    queryFn: () => fetchTurnos(empresaId, true),
  });
  const turnos = q.data ?? [];
  if (q.isLoading)
    return (
  ...
  if (!turnos.length)
    return (
      <Card className="p-6 text-center text-sm text-muted-foreground">
        Aún no hay cortes (turnos cerrados).
      </Card>
    );
  return (
    <Card className="overflow-x-auto">
      <table className="w-full text-sm">
        <thead className="bg-muted/50 text-xs">
          <tr>
            ...
            <th className="text-right px-3 py-2">Diferencia</th>
            <th className="text-center px-3 py-2">Recepción</th>
            <th className="text-center px-3 py-2"></th>
          </tr>
        </thead>
        <tbody>
          {turnos.map((t: any) => (
            ...
            >
              ...
              <td className="px-3 py-2">{t.cajero_nombre}</td>
              <td className="px-3 py-2 text-xs tabular-nums">
                {fmtDate(t.cerrado_at)}
              </td>
              <td className="px-3 py-2 text-right tabular-nums">
                {_fmt(t.total_efectivo_esperado ?? 0)}
              </td>
              <td className="px-3 py-2 text-right tabular-nums">
                {_fmt(t.total_efectivo_contado ?? 0)}
              </td>
              <td className="px-3 py-2 text-right tabular-nums">
                {_fmt(t.total_tarjeta_contado ?? 0)}
              </td>
              <td className="px-3 py-2 text-right tabular-nums">
                {_fmt(t.total_transferencia_contado ?? 0)}
              </td>
              <td className="px-3 py-2 text-right tabular-nums">
                {_fmt(t.total_otros_contado ?? 0)}
              </td>
              <td
                className={`px-3 py-2 text-right tabular-nums font-semibold ${Number(t.diferencia ?? 0) === 0 ? "text-success" : Number(t.diferencia) < 0 ? "text-destructive" : "text-warning"}`}
              >
                {_fmt(t.diferencia ?? 0)}
              </td>
              <td className="px-3 py-2 text-center">
                {t.recibido_tesoreria ? (
                  <Badge
                    variant="outline"
...
function MovimientosPanel({
  ...
  const q = useQuery({
    ...
    queryFn: async () => {
      const { data } = await supabase
        .from("caja_movimientos")
        .select(
          "id, tipo, monto, motivo, created_at, turno_id, user_id, caja_turnos(caja_nombre)",
        )
        .eq("empresa_id", empresaId)
        .eq("tipo", tipo)
      ...
      return (data ?? []).map((r: any) => ({
        ...r,
        user_nombre: nameMap.get(r.user_id) ?? "—",
      }));
    },
  });
    ...
    return allRows.filter((r) => {
      if (
        ...
        !(
          ...
          (r.user_nombre || "").toLowerCase().includes(s) ||
          (r.caja_turnos?.caja_nombre || "").toLowerCase().includes(s)
        )
      )
        return false;
  ...
  return (
    <div className="space-y-3">
      <SumCard
        icon={icon}
        label={`Total ${labelTipo} (${rows.length})`}
        value={total}
        tone={tone}
      />
      <FiltersBar
        ...
        to={to}
        onTo={setTo}
        placeholder="Buscar por motivo, usuario o caja..."
      />
      {!rows.length ? (
        <Card className="p-6 text-center text-sm text-muted-foreground">
          Sin {labelTipo.toLowerCase()} registrados.
        </Card>
      ) : (
        <Card className="overflow-x-auto">
          <table className="w-full text-sm">
...
function VentasPosPanel({ empresaId }: { empresaId: string }) {
  ...
  const q = useQuery({
    ...
    queryFn: async () => {
      const { data } = await supabase
        .from("ventas")
        .select(
          "id, folio, fecha, created_at, total, status, condicion_pago, turno_id, vendedor_id, cliente:clientes(nombre), caja_turnos(caja_nombre, cajero_id)",
        )
        .eq("empresa_id", empresaId)
        .eq("origen", "pos")
        .order("created_at", { ascending: false })
      ...
      const vendedorIds = Array.from(
        new Set(rows.map((r) => r.vendedor_id).filter(Boolean)),
      );
      const cajeroUserIds = Array.from(
        new Set(rows.map((r) => r.caja_turnos?.cajero_id).filter(Boolean)),
      );
      ...
      const [profilesById, profilesByUserId] = await Promise.all([
        ...
        cajeroUserIds.length
          ? supabase
              .from("profiles")
              .select("user_id, nombre")
              .in("user_id", cajeroUserIds)
          : Promise.resolve({ data: [] as any[] }),
      ]);
      const vendMap = new Map(
        (profilesById.data ?? []).map((p: any) => [p.id, p.nombre]),
      );
      const cajMap = new Map(
        (profilesByUserId.data ?? []).map((p: any) => [p.user_id, p.nombre]),
      );
  ...
  const rows = useMemo(() => {
    ...
    return allRows.filter((r) => {
      if (s) {
        const hay =
          `${r.folio || ""} ${r.cliente?.nombre || ""} ${r.vendedor_nombre || ""} ${r.cajero_nombre || ""} ${r.caja_turnos?.caja_nombre || ""}`.toLowerCase();
      ...
      if (metodoFilter !== "all" && !r.metodos_pago.includes(metodoFilter))
        return false;
      return true;
    });
  ...
  return (
    <div className="space-y-3">
      <SumCard
        ...
        label={`Total ventas POS (${rows.length})`}
        value={total}
        tone="primary"
      />
      <FiltersBar
      ...
      {!rows.length ? (
        <Card className="p-6 text-center text-sm text-muted-foreground">
          Sin ventas POS que coincidan con los filtros.
        </Card>
      ) : (
        <Card className="overflow-x-auto">
          <table className="w-full text-sm">
                  ...
                  <>
                    ...
                    >
                      ...
                      <td className="px-3 py-2">
                        <Badge
                          variant="outline"
                          className={`text-xs capitalize ${isCancelled ? "bg-destructive/10 text-destructive border-destructive/30" : ""}`}
                        >
                          {r.status}
                        </Badge>
                      </td>
                      <td
                        className={`px-3 py-2 text-right font-semibold tabular-nums ${isCancelled ? "line-through text-muted-foreground" : ""}`}
                      >
                        {_fmt(r.total)}
                      </td>
                    </tr>
                    {isExpanded && (
                      <VentaExpandedRow ventaId={r.id} fmt={_fmt} />
                    )}
                  </>
                );
              })}
    ...
    queryFn: async () => {
      const [lRes, pRes, hRes] = await Promise.all([
        supabase
          .from("venta_lineas")
          .select(
            "id, cantidad, precio_unitario, descuento_pct, subtotal, iva_monto, total, productos(nombre, unidad_granel)",
          )
          .eq("venta_id", ventaId)
          .order("created_at"),
        supabase
        ...
        <div className="px-5 py-3">
          {q.isLoading ? (
            <div className="text-xs text-muted-foreground py-2 flex items-center gap-2">
              <Loader2 className="h-3 w-3 animate-spin" /> Cargando detalles...
            </div>
          ) : (
            <Tabs defaultValue="productos">
              <TabsList className="h-8">
                <TabsTrigger value="productos" className="text-xs h-7">
                  Productos ({q.data?.lineas.length ?? 0})
                </TabsTrigger>
                <TabsTrigger value="pagos" className="text-xs h-7">
                  Pagos ({q.data?.pagos.length ?? 0})
                </TabsTrigger>
                <TabsTrigger value="historial" className="text-xs h-7">
                  Historial ({q.data?.historial.length ?? 0})
                </TabsTrigger>
              </TabsList>
                  ...
                  <table className="w-full text-xs">
                    <thead className="text-muted-foreground">
                      <tr className="border-b border-border/50">
                        ...
                        <th className="text-right py-1.5 font-medium w-16">
                          Cant
                        </th>
                        <th className="text-center py-1.5 font-medium w-12">
                          Ud
                        </th>
                    ...
                    <tbody>
                      {q.data.pagos.map((p: any) => (
                        <tr key={p.id} className="border-b border-border/30">
                          ...
                          <td className="py-1.5 text-muted-foreground">
                            {fmtDate(p.cobros?.fecha)}
                          </td>
                          <td className="py-1.5 text-right font-semibold tabular-nums">
                            {fmt(p.monto_aplicado)}
                          </td>
                        </tr>
                      ))}
                    </tbody>
...
function TurnoDetalleModal({
  ...
  const q = useQuery({
    ...
    queryFn: async () => {
      ...
      const { data: ventas } = await supabase
        .from("ventas")
        .select(
          "id, folio, fecha, created_at, total, condicion_pago, status, vendedor_id, cliente:clientes(nombre)",
        )
        .eq("turno_id", turnoId)
        .order("created_at", { ascending: false });
      // vendedor_id refers to profiles.id
      const vendedorIds = Array.from(
        new Set((ventas ?? []).map((v: any) => v.vendedor_id).filter(Boolean)),
      );
      const { data: profiles } = vendedorIds.length
        ...
        : { data: [] as any[] };
      const nameMap = new Map(
        (profiles ?? []).map((p: any) => [p.id, p.nombre]),
      );
      ...
      if (t && t.status === "abierto") {
        ...
        t = {
          ...t,
          total_efectivo_esperado: Math.max(0, efectivo),
          total_tarjeta_esperado: tarjeta,
          total_transferencia_esperado: transferencia,
          total_otros_esperado: otros,
        };
      }
  ...
  const marcarRecibido = async () => {
    ...
    toast.success("Corte marcado como recibido");
    q.refetch();
  };
  return (
    <Dialog open={open} onOpenChange={(v) => !v && onClose()}>
      <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto">
        ...
        {q.isLoading || !t ? (
          ...
          (() => {
            const totalEfectivoEsperado =
              Number(t.total_efectivo_esperado) || 0;
            ...
            const totalTransferenciaEsperado =
              Number(t.total_transferencia_esperado) || 0;
            const totalTransferenciaContado =
              Number(t.total_transferencia_contado) || 0;
            const difTransferencia =
              totalTransferenciaContado - totalTransferenciaEsperado;
            ...
            const totalEsperado =
              ...
              totalTarjetaEsperado +
              totalTransferenciaEsperado +
              totalOtrosEsperado;
            const totalContado =
              totalEfectivoContado +
              totalTarjetaContado +
              totalTransferenciaContado +
              totalOtrosContado;
              ...
              <div className="space-y-4">
                <div className="rounded-lg border border-border p-3 flex items-center justify-between gap-3">
                  <div>
                    ...
                    <p className="text-xs text-muted-foreground">
                      {t.recibido_tesoreria
                        ? `Recibido el ${fmtDate(t.recibido_at)}`
                        : "Pendiente de recibir/verificar"}
                    </p>
                  </div>
                  ...
                  {t.recibido_tesoreria ? (
                    ...
                    <Button size="sm" onClick={marcarRecibido}>
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-2">
                  <Kpi
                    ...
                    label="Fondo inicial"
                    value={Number(t.fondo_inicial) || 0}
                    tone="primary"
                  />
                  <Kpi
                    icon={<Banknote className="h-3.5 w-3.5" />}
                  ...
                  <table className="w-full text-xs text-left">
                    ...
                    <tbody className="divide-y">
                      <tr>
                        ...
                        <td className="px-3 py-2 text-right tabular-nums">
                          {_fmt(totalEfectivoEsperado)}
                        </td>
                        <td className="px-3 py-2 text-right tabular-nums">
                          {t.status === "abierto"
                            ? "—"
                            : _fmt(totalEfectivoContado)}
                        </td>
                        <td
                          className={`px-3 py-2 text-right tabular-nums font-semibold ${t.status === "abierto" ? "text-muted-foreground" : difEfectivo === 0 ? "text-success" : difEfectivo < 0 ? "text-destructive" : "text-warning"}`}
                        >
                          {t.status === "abierto"
                            ? "—"
                            : (difEfectivo > 0 ? "+" : "") + _fmt(difEfectivo)}
                        </td>
                      </tr>
                      <tr>
                        <td className="px-3 py-2 flex items-center gap-1.5 font-medium">
                        ...
                        <td className="px-3 py-2 text-right tabular-nums">
                          {_fmt(totalTarjetaEsperado)}
                        </td>
                        <td className="px-3 py-2 text-right tabular-nums">
                          {t.status === "abierto"
                            ? "—"
                            : _fmt(totalTarjetaContado)}
                        </td>
                        <td
                          className={`px-3 py-2 text-right tabular-nums font-semibold ${t.status === "abierto" ? "text-muted-foreground" : difTarjeta === 0 ? "text-success" : difTarjeta < 0 ? "text-destructive" : "text-warning"}`}
                        >
                          {t.status === "abierto"
                            ? "—"
                            : (difTarjeta > 0 ? "+" : "") + _fmt(difTarjeta)}
                        </td>
                      </tr>
                      <tr>
                        <td className="px-3 py-2 flex items-center gap-1.5 font-medium">
                        ...
                        <td className="px-3 py-2 text-right tabular-nums">
                          {_fmt(totalTransferenciaEsperado)}
                        </td>
                        <td className="px-3 py-2 text-right tabular-nums">
                          {t.status === "abierto"
                            ? "—"
                            : _fmt(totalTransferenciaContado)}
                        </td>
                        <td
                          className={`px-3 py-2 text-right tabular-nums font-semibold ${t.status === "abierto" ? "text-muted-foreground" : difTransferencia === 0 ? "text-success" : difTransferencia < 0 ? "text-destructive" : "text-warning"}`}
                        >
                          {t.status === "abierto"
                            ? "—"
                            : (difTransferencia > 0 ? "+" : "") +
                              _fmt(difTransferencia)}
                        </td>
                      </tr>
                      <tr>
                        <td className="px-3 py-2 flex items-center gap-1.5 font-medium">
                        ...
                        <td className="px-3 py-2 text-right tabular-nums">
                          {_fmt(totalOtrosEsperado)}
                        </td>
                        <td className="px-3 py-2 text-right tabular-nums">
                          {t.status === "abierto"
                            ? "—"
                            : _fmt(totalOtrosContado)}
                        </td>
                        <td
                          className={`px-3 py-2 text-right tabular-nums font-semibold ${t.status === "abierto" ? "text-muted-foreground" : difOtros === 0 ? "text-success" : difOtros < 0 ? "text-destructive" : "text-warning"}`}
                        >
                          {t.status === "abierto"
                            ? "—"
                            : (difOtros > 0 ? "+" : "") + _fmt(difOtros)}
                        </td>
                      </tr>
                    </tbody>
                  </table>
                ...
                <div className="text-xs text-muted-foreground grid grid-cols-2 gap-2">
                  <div>
                    <b>Apertura:</b> {fmtDate(t.abierto_at)}
                  </div>
                  <div>
                    <b>Cierre:</b> {t.cerrado_at ? fmtDate(t.cerrado_at) : "—"}
                  </div>
                  {t.notas_apertura && (
                    <div className="col-span-2">
                      <b>Notas apertura:</b> {t.notas_apertura}
                    </div>
                  )}
                  {t.notas_cierre && (
                    <div className="col-span-2">
                      <b>Notas cierre:</b> {t.notas_cierre}
                    </div>
                  )}
                </div>
                <div>
                  <h4 className="text-sm font-semibold mb-2 flex items-center gap-1.5">
                    <Banknote className="h-4 w-4" /> Movimientos (
                ...
                <div>
                  <h4 className="text-sm font-semibold mb-2 flex items-center gap-1.5">
                    <ShoppingCart className="h-4 w-4" /> Ventas del turno (
                    {q.data?.ventas.length ?? 0})
                  </h4>
                  <Card className="overflow-x-auto">
                    <table className="w-full text-xs">
                      <thead className="bg-muted/50">
                        <tr>
                          ...
                          <th className="text-left px-2 py-1.5">Vendedor</th>
                          <th className="text-left px-2 py-1.5">Condición</th>
                          <th className="text-left px-2 py-1.5">Métodos</th>
