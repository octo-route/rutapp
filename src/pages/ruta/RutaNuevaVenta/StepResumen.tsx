      ...
      <div className="flex-1 overflow-auto px-3 pt-2.5 pb-24 space-y-2.5">
        <div className="flex items-center gap-2 bg-card rounded-lg px-3 py-2.5">
          ...
          <button
            ...
            className="text-[10.5px] text-primary font-medium"
          >
            Cambiar
          </button>
        </div>
        {devoluciones.length > 0 && (
          <section className="bg-card rounded-lg p-3">
            ...
            {devoluciones.map((d) => (
              <div
                key={d.producto_id}
                className="flex items-center justify-between py-1 border-b border-border/40 last:border-0 text-[11px]"
              >
                ...
                <div className="flex items-center gap-1 shrink-0">
                  <span
                    className={`px-1.5 py-0.5 rounded text-[9px] font-medium ${d.accion === "reposicion" ? "bg-primary/10 text-primary" : d.accion === "descuento_venta" ? "bg-amber-100 text-amber-700 dark:bg-amber-950/40 dark:text-amber-400" : "bg-accent text-muted-foreground"}`}
                  >
          ...
          <section className="bg-emerald-50 dark:bg-emerald-950/30 border border-emerald-200 dark:border-emerald-800 rounded-lg p-3">
            ...
            {promoResults.map((r, i) => (
              <div key={i} className="flex justify-between text-[11px] py-0.5">
                <span className="text-emerald-700 dark:text-emerald-300 truncate flex-1 mr-2">
                  {r.descripcion}
                </span>
                {r.descuento > 0 && (
                ...
                {r.cantidad_gratis && r.cantidad_gratis > 0 && (
                  <span className="text-emerald-600 dark:text-emerald-400 font-semibold shrink-0">
                    {r.cantidad_gratis}x gratis
                  </span>
                )}
              </div>
            ))}
        ...
        <section className="bg-card rounded-lg p-3">
          <div className="space-y-1">
            ...
            {totals.descuento -
              (totals.descuentoDevolucion ?? 0) -
              descExtraAmt >
              0 && (
              <div className="flex justify-between text-[12px]">
                <span className="text-emerald-600 dark:text-emerald-400 flex items-center gap-1">
                  <Tag className="h-3 w-3" /> Promociones
                </span>
            ...
            <div className="flex items-center gap-2">
              ...
              <div className="flex-1">
                <p className="text-[11px] font-semibold text-amber-800 dark:text-amber-300">
                  Cuentas pendientes: {fmt(saldoPendienteTotal)}
                </p>
                <p className="text-[10px] text-amber-600 dark:text-amber-400">
                  Podrás aplicar pagos en el siguiente paso
                </p>
              </div>
            </div>
          </div>
