...
import {
  useCotizacion,
  useSaveCotizacion,
  useSaveCotizacionLinea,
  useDeleteCotizacionLinea,
  useDeleteCotizacion,
  useConvertCotizacionToVenta,
...
export function emptyLine(): Partial<CotizacionLinea> & {
  unidad_label?: string;
...
export function useCotizacionForm() {
  ...
  const { data: existingCotizacion, isLoading } = useCotizacion(
    isNew ? undefined : id,
  );
  ...
  const [lineas, setLineas] = useState<Partial<CotizacionLinea>[]>([
    emptyLine(),
  ]);
  ...
  const readOnly = isNew
    ? !canCreateCotizacion
    : !!(form as any).venta_id || !canEditCotizacion;
  ...
  const effectiveListaId =
    (form as any).lista_precio_id || defaultListaPrecioData?.id || null;
  // Use tarifa from form, or fall back to the tarifa linked to the default lista
  const effectiveTarifaId =
    (form as any).tarifa_id || defaultListaPrecioData?.tarifa_id || null;

  // Tarifa rules
  const { data: tarifaRules } = useQuery({
    queryKey: ["tarifa-rules-cotizacion", effectiveTarifaId],
    ...
    queryFn: async () => {
      const { data, error } = await supabase
        .from("tarifa_lineas")
        .select(
          "aplica_a, producto_ids, clasificacion_ids, presentacion_ids, tipo_calculo, precio, precio_minimo, margen_pct, descuento_pct, redondeo, base_precio, lista_precio_id",
        )
  ...
  const productosList = useMemo(() => {
    ...
    return productosListRaw.map((p: any) => {
      ...
      if (tarifaRules?.length) {
        ...
        dispPrice = buildSalePricingSnapshot(
          prodForPricing,
          pricing,
        ).displayPrice;
      }
  ...
  useEffect(() => {
    ...
    const existingLines = (existingCotizacion.cotizacion_lineas ?? []).map(
      (l: any) => {
  ...
  const totals = useMemo(() => {
    ...
    lineas.forEach((l) => {
      const qty = Number(l.cantidad) || 0,
        ...
        desc = Number(l.descuento_pct) || 0;
      const lineSubtotal = r2(qty * price),
        discountAmt = r2(lineSubtotal * (desc / 100)),
        base = r2(lineSubtotal - discountAmt);
      const ieps = r2(base * ((Number(l.ieps_pct) || 0) / 100)),
        iva = r2((base + ieps) * ((Number(l.iva_pct) || 0) / 100));
      iva_total += iva;
      ieps_total += ieps;
    ...
    return {
      ...
      descuento_total: r2(descuento_total),
  ...
  useEffect(() => {
    ...
    setLineas((prev) =>
      prev.map((l) => {
        ...
        const prodForPricing: ProductForPricing = {
          id: l.producto_id,
          precio_principal: Number(prod.precio_principal) || 0,
          costo: Number(prod.costo) || 0,
          costos_adicionales: prod.costos_adicionales,
          clasificacion_id: prod.clasificacion_id,
          tiene_iva: prod.tiene_iva,
          iva_pct: Number(prod.iva_pct ?? 16),
          tiene_ieps: prod.tiene_ieps || Number(prod.ieps_pct) > 0,
          ieps_pct: Number(prod.ieps_pct ?? 0),
  ...
  const handleProductSelect = (idx: number, productoId: string) => {
    ...
    const unidadId =
      producto.unidad_cotizacion_id || producto.unidad_compra_id || null;
    ...
    if (hasIeps)
      taxes.push(
        producto.ieps_tipo === "cuota" ? "IEPS cuota" : `IEPS ${iepsPct}%`,
      );
    ...
    const pricing = tarifaRules?.length
      ? resolveProductPricing(tarifaRules, prodForPricing, effectiveListaId)
      : null;
    const snap = pricing
      ? buildSalePricingSnapshot(prodForPricing, pricing)
      : null;
    const finalUnitPrice = snap
      ? snap.unitPrice
      : Number(producto.precio_principal) || 0;

    // Si tiene presentaciones, abrir el modal
    const prodPresentaciones =
      presentacionesList?.filter(
        (p: any) => p.producto_id === productoId && p.activo,
      ) || [];
  ...
  const handleConfirmPresentacion = (data: {
    cantidadBase: number;
    paquetes: number | null;
    presentacion: any | null;
    pricing: any;
  }) => {
  ...
  const handleEditPresentacion = (idx: number) => {
    ...
    const prodPresentaciones =
      presentacionesList?.filter(
        (p: any) => p.producto_id === l.producto_id && p.activo,
      ) || [];
  ...
  const navigateCell = useCallback(
    (rowIdx: number, colIdx: number, dir: "next" | "prev") => {
      if (dir === "next") {
  ...
  const updateLine = (idx: number, field: string, val: any) => {
    ...
    if ((field === "iva_pct" || field === "ieps_pct") && tarifaRules?.length) {
      ...
      if (line?.producto_id) {
        ...
        if (prod) {
          const newIvaPct =
            field === "iva_pct" ? Number(val) : Number(line.iva_pct);
          const newIepsPct =
            field === "ieps_pct" ? Number(val) : Number(line.ieps_pct);
          const prodForPricing: ProductForPricing = {
            id: line.producto_id!,
            precio_principal: Number(prod.precio_principal) || 0,
            costo: Number(prod.costo) || 0,
            clasificacion_id: prod.clasificacion_id,
            tiene_iva: newIvaPct > 0,
            iva_pct: newIvaPct > 0 ? newIvaPct : Number(prod.iva_pct ?? 16),
            tiene_ieps: newIepsPct > 0,
            ieps_pct: newIepsPct > 0 ? newIepsPct : Number(prod.ieps_pct ?? 0),
            ieps_tipo: prod.ieps_tipo,
            usa_listas_precio: prod.usa_listas_precio,
            costos_adicionales: prod.costos_adicionales,
          };
    ...
    try {
      ...
      for (const l of lineas) {
        ...
        const lineSubtotal = qty * price,
          discountAmt = lineSubtotal * (desc / 100),
          base = lineSubtotal - discountAmt;
      ...
      if (isNew)
        navigate(`/ventas/cotizaciones/${cotizacionId}`, { replace: true });
      setDirty(false);
      return cotizacionId;
    } catch (e: any) {
  ...
  const handleDelete = async () => {
    if (!form.id) return;
    await deleteCotizacion.mutateAsync(form.id);
  ...
  const handleConvertToVenta = async (almacenId?: string) => {
    if (!form.id) return;
    try {
      const ventaId = await convertToVenta.mutateAsync({
        cotizacionId: form.id,
        almacenId,
      });
  ...
  return {
    ...
    setProductBeingConfigured,
    handleConfirmPresentacion,
    handleEditPresentacion,
    profile,
    user,
    empresa,
    navigate,
