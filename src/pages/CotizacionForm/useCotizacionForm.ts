...
export function useCotizacionForm() {
  ...
  const handleProductSelect = (idx: number, productoId: string) => {
    ...
    setLineas(prev => { const next = [...prev]; next[idx] = { ...next[idx], producto_id: productoId, descripcion: producto.nombre, precio_unitario: finalUnitPrice, unidad_id: unidadId, iva_pct: ivaPct, ieps_pct: iepsPct, unidad_label: unidadLabel, impuestos_label: taxes.join(', '), precio_manual: false } as any; return next; });
    setDirty(true);
  };
