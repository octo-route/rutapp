...
export default function CotizacionFormPage() {
  ...
  const vendedorOptions = (vendedoresList ?? []).map((v) => ({
    value: v.id,
    label: v.nombre,
  }));
  const clienteNombre = clientesList?.find(
  ...
  const onClienteChange = (cId: string) => {
    ...
    const clienteTarifa =
      c?.tarifa_id || tarifasList?.find((t) => t.tipo === "general")?.id;

    if (clienteTarifa) set("tarifa_id", clienteTarifa);

    if (c && (c as any).lista_precio_id) {
      set("lista_precio_id", (c as any).lista_precio_id);
    } else {
    ...
    <div className="min-h-full">
      ...
      <div className="bg-card border-b border-border sticky top-0 z-20 flex items-center justify-between px-3 sm:px-5 h-14">
        <div className="flex items-center gap-3">
          <button
            ...
            className="p-1.5 -ml-1.5 hover:bg-muted rounded text-muted-foreground transition-colors"
          >
          ...
          {!readOnly && (
            ...
            >
              ...
              <span className="hidden sm:inline">
                {saveCotizacion.isPending ? "Guardando..." : "Guardar"}
              </span>
            </button>
          )}
      ...
      {!isNew && (
        <div className="px-5 pt-3">
          <OdooStatusbar
            steps={COTIZACION_STEPS}
            current={form.status as string}
            onStepClick={
              !canEditCotizacion || (form as any).venta_id
                ? undefined
                : (k) => handleStatusChange(k as StatusCotizacion)
            }
          />
        </div>
      )}
        ...
        <div className="bg-card border border-border rounded-md p-5">
          {readOnly && (
            <div className="mb-3 text-xs text-muted-foreground bg-muted/60 border border-border px-3 py-2 rounded flex items-center gap-2">
              ...
              Esta cotización está {form.status} y no se puede editar.
            </div>
          )}
          {(form as any).venta_id && (
            <div className="mb-3 text-sm font-medium text-primary bg-primary/10 border border-primary/20 px-3 py-2 rounded flex items-center gap-2">
              Esta cotización fue convertida a la venta:{" "}
              {(form as any).ventas?.folio || "Ver Venta"}
            </div>
          )}
            ...
            tabs={[
              {
                ...
                content: (
                  <CotizacionLineasTab
                    ...
                    setLineas={setLineas}
                    onEditPresentacion={handleEditPresentacion}
      ...
      <AlertDialog open={showDeleteConfirm} onOpenChange={setShowDeleteConfirm}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              Esta acción no se puede deshacer. La cotización y todas sus líneas
      ...
      >
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              Se creará una nueva venta en estado "borrador" con los datos de
              esta cotización. Puedes seleccionar un almacén para afectar el
              inventario al confirmar la venta.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <div className="py-4">
      ...
      <DocumentPreviewModal
        ...
        defaultPhone={
          clientesList?.find((c) => c.id === form.cliente_id)?.telefono ?? ""
        }
