...
import { CotizacionLineaMobile } from './CotizacionLineaMobile';
import { CotizacionLineaDesktop } from './CotizacionLineaDesktop';
    ...
    <div className="p-3 sm:p-4 space-y-3">
      {isMobile ? (
        <div className="space-y-2">
          {lineas.map((l, idx) => (
            <CotizacionLineaMobile key={idx} idx={idx} line={l} {...props} currencySymbol={symbol} />
          ))}
        </div>
      ) : (
        <div>
          <table className="w-full text-[13px]">
            ...
            <tbody>
              {lineas.map((l, idx) => (
                <CotizacionLineaDesktop key={idx} idx={idx} line={l} isLast={idx === lineas.length - 1} {...props} currencySymbol={symbol} />
              ))}
            </tbody>
          </table>
        </div>
