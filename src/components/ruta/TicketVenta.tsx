...
export default function TicketVenta(props: TicketVentaProps) {
  ...
  const [taxMode, setTaxMode] = useState<"ambos" | "totales" | "ninguno">(
    "ambos",
  );
  ...
  const ahorroCliente = lineas.reduce((sum, l) => {
    const sugerido = Number(l.precio_sugerido_publico) || 0;
    if (sugerido <= 0) return sum;

    return sum + Math.max(0, sugerido * l.cantidad - l.total);
  }, 0);
  ...
  const handlePrint = () => {
    ...
    printWindow.document
      .write(`<!DOCTYPE html><html><head><meta charset="utf-8"/><title>Ticket ${folio}</title>
<style>
...
<script>window.onload=function(){window.print();window.close()}</script></body></html>`);
  ...
  const handleShare = async () => {
    const text = [
      ...
      descuentoDevolucion > 0
        ? `Desc. devolución: -${fmt(descuentoDevolucion)}`
        : "",
      `TOTAL: ${fmt(total)}`,
      ...(devoluciones.length > 0
        ? [
            ...
            ...devoluciones.map(
              (d) =>
                `  ${d.cantidad}x ${d.nombre} → ${ACCION_LABELS[d.accion] || d.accion} (${MOTIVO_LABELS[d.motivo] || d.motivo})`,
            ),
          ]
        : []),
    ...
    <div className="min-h-screen bg-background flex flex-col">
      ...
      <div className="sticky top-0 z-10 bg-background border-b border-border px-4 py-3 flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          <button
            onClick={onPrintTicket ?? handlePrint}
          ...
          <div ref={ticketRef}>
            ...
            <div className="tk-center px-5 pt-4 pb-2">
              {empresa.logo_url && (
                <img
                  ...
                  alt={empresa.nombre}
                  className="tk-logo h-8 max-w-[120px] object-contain mx-auto mb-1"
                />
              )}
              <p className="tk-empresa text-[12px] font-bold text-foreground">
                {empresa.nombre}
              </p>
              {empresa.razon_social && (
                <p className="tk-sub text-[9px] text-muted-foreground">
                  {empresa.razon_social}
                </p>
              )}
              {empresa.rfc && (
              ...
              {(() => {
                const dir1 = [empresa.direccion, empresa.colonia]
                  .filter(Boolean)
                  .join(", ");
                return dir1 ? (
                  <p className="tk-sub text-[8px] text-muted-foreground mt-px">
                    {dir1}
                  </p>
                ) : null;
              })()}
              {(() => {
                const dir2 = [
                  ...
                  .join(", ");
                return dir2 ? (
                  <p className="tk-sub text-[8px] text-muted-foreground">
                    {dir2}
                  </p>
                ) : null;
              })()}
            ...
            <div className="px-5 py-2 space-y-0.5 text-[10px]">
              <div className="flex gap-4">
                ...
                <span>
                  <span className="font-bold text-foreground">Fecha </span>
                  <span className="text-muted-foreground">{fecha}</span>
                </span>
              </div>
              <div>
                <span className="font-bold text-foreground">Cliente </span>
                <span className="text-muted-foreground">{clienteNombre}</span>
              </div>
              {vendedorNombre && (
                <div>
              ...
              <div className="flex gap-4">
                <span>
                  ...
                  <span className="text-muted-foreground">{pagoLabel}</span>
                </span>
                {condicionPago !== "credito" && metodoPago && (
                  <span>
                    <span className="font-bold text-foreground">Método </span>
            ...
            <div className="px-5 py-2">
              <p className="tk-section text-[8px] font-bold text-muted-foreground uppercase tracking-wider mb-1.5">
                Productos
              </p>
              <div className="space-y-1">
                {lineas.map((l, i) => (
                  <div
                    key={i}
                    className={`py-0.5 ${l.esCambio ? "opacity-60" : ""}`}
                  >
                    <div className="flex justify-between items-baseline text-[11px]">
                      <span className="text-foreground font-medium flex-1 mr-2 truncate">
                        {l.cantidad}x {l.nombre}
                        {l.esCambio && (
                          <span className="text-muted-foreground text-[9px] ml-1 italic">
                            (cambio)
                          </span>
                        )}
                      </span>
                        ...
                        <div className="flex gap-2 text-[8px] text-muted-foreground mt-px flex-wrap">
                          ...
                          {(l.precio_sugerido_publico ?? 0) > 0 && (
                            <span className="text-primary font-medium">
                              Sug. público {fmt(l.precio_sugerido_publico!)}
                            </span>
                          )}
                        </div>
                        {promociones
                          .filter(
                            (p) =>
                              p.producto_id && p.producto_id === l.producto_id,
                          )
                          .map((p, pi) => (
                            <div
                              key={pi}
                              className="flex justify-between text-[8px] mt-px"
                            >
            ...
            {promociones.length > 0 &&
              promociones.reduce((s, p) => s + p.descuento, 0) > 0 && (
                <div className="px-5 py-0.5">
                  <div className="flex justify-between text-[10px]">
                    ...
                    <span className="text-primary font-bold tabular-nums">
                      -{fmt(promociones.reduce((s, p) => s + p.descuento, 0))}
                    </span>
                  </div>
                </div>
              )}
            ...
            <div className="px-5 py-2 space-y-0.5">
              {taxMode !== "ninguno" && (
                <div className="tk-tot-row flex justify-between text-[10px]">
                  ...
                  <span className="val text-foreground tabular-nums">
                    {fmt(subtotal)}
                  </span>
                </div>
              )}
              ...
              {descuentoDevolucion > 0 && (
                <div className="tk-tot-row flex justify-between text-[10px]">
                  ...
                  <span className="val text-amber-600 font-medium tabular-nums">
                    -{fmt(descuentoDevolucion)}
                  </span>
                </div>
              )}
              ...
              {ahorroCliente > 0 && (
                <div className="tk-tot-row flex justify-between text-[10px]">
                  <span className="lbl text-muted-foreground">Ahorraste</span>
                  <span className="val text-green-600 font-bold tabular-nums">
                    {fmt(ahorroCliente)}
                  </span>
                </div>
              )}
                ...
                <div className="pt-1 space-y-0.5">
                  ...
                  {(cambio ?? 0) > 0 && (
                    <div className="tk-tot-row flex justify-between text-[10px]">
                      ...
                      <span className="val text-primary font-bold tabular-nums">
                        {fmt(cambio!)}
                      </span>
                    </div>
                  )}
              ...
              <>
                ...
                <div className="px-5 py-2 space-y-0.5">
                  ...
                  <div className="flex justify-between text-[11px] font-bold pt-1 border-t border-dashed border-border">
                    ...
                    <span
                      className={`tabular-nums ${(saldoNuevo ?? 0) > 0 ? "text-destructive" : "text-green-600"}`}
                    >
                      {fmt(saldoNuevo ?? 0)}
                    </span>
                  </div>
