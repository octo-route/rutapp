...
export default function CrearConteoDialog({ open, onClose, onCreated }: Props) {
  ...
  const [clasificacionId, setClasificacionId] = useState("todas");
  ...
  const { data: previewCount } = useQuery({
    ...
    queryFn: async () => {
      ...
      let q = supabase
        .from("stock_almacen")
        .select(
          "producto_id, cantidad, productos!inner(id, status, clasificacion_id)",
        )
      ...
      if (filtroStock === "positivo")
        items = items.filter((i: any) => i.cantidad > 0);
      else if (filtroStock === "negativo")
        items = items.filter((i: any) => i.cantidad < 0);
      else if (filtroStock === "no_cero")
        items = items.filter((i: any) => i.cantidad !== 0);
  ...
  const handleCreate = async () => {
    ...
    try {
      ...
      let q = supabase
        .from("stock_almacen")
        .select(
          "producto_id, cantidad, productos!inner(id, status, clasificacion_id, costo)",
        )
        .eq("almacen_id", almacenId)
        .eq("empresa_id", empresa!.id)
        .eq("productos.status", "activo" as any);
      ...
      if (items.length === 0) {
        toast.error("No hay productos con los filtros seleccionados");
        setSaving(false);
        return;
      }

      // Insert conteo
      const { data: conteo, error: err1 } = await supabase
        .from("conteos_fisicos")
        .insert({
          folio,
          ...
          creado_por: user!.id,
          clasificacion_id:
            clasificacionId === "todas" ? null : clasificacionId,
