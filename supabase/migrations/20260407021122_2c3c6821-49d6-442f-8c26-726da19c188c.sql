...
-- Fix backfilled pedido movements: set vendedor_destino_id for pedidos that have a vendedor
UPDATE movimientos_inventario mi
SET vendedor_destino_id = v.vendedor_id,
    referencia_tipo = 'venta_ruta'
FROM ventas v
WHERE mi.referencia_id = v.id
  ...
  AND mi.tipo = 'salida'
  AND mi.vendedor_destino_id IS NULL
  AND mi.notas LIKE '%Pedido entregado%';

-- Recalculate stock_camion
...
-- Also update trigger to always use vendedor_destino_id when vendedor exists (not just when carga active)
...
begin
  ...
  -- Check for active carga to decide stock source
  ...
  loop
    ...
    if v_has_active_carga then
      ...
      where vendedor_id = new.vendedor_id and producto_id = v_linea.producto_id for update;
      ...
      if not coalesce(v_vender_sin_stock, false) and v_new_qty < 0 then
        raise exception 'Stock insuficiente en camión para "%"', 
    ...
    -- Always record movement with vendedor if available (for Kardex visibility)
    insert into public.movimientos_inventario 
      (id, empresa_id, tipo, producto_id, cantidad, vendedor_destino_id, almacen_origen_id, referencia_tipo, referencia_id, user_id, fecha, created_at, notas)
    values 
      (gen_random_uuid(), new.empresa_id, 'salida', v_linea.producto_id, v_linea.cantidad, 
       new.vendedor_id,
       CASE WHEN NOT v_has_active_carga THEN new.almacen_id ELSE NULL END,
       CASE WHEN new.vendedor_id IS NOT NULL THEN 'venta_ruta' ELSE 'venta' END,
       new.id, coalesce(new.vendedor_id, new.cliente_id), 
