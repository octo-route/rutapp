...
begin
  ...
  if v_has_active_carga then
    -- Vendedor has active carga: register salida from vendedor's truck stock
  ...
  if v_venta.almacen_id is null then
    -- No almacen assigned — register movement without location for traceability
    insert into public.movimientos_inventario 
  ...
  -- POS sale: deducted from almacen, movement links to almacen only (not vendedor)
