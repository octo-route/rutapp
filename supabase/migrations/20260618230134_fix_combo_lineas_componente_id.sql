-- Fix apply_immediate_sale_inventory to use componente_id from combo_lineas
...
begin
  ...
  if v_es_combo then
    ...
    for v_combo_line in select * from public.combo_lineas where combo_id = new.producto_id loop
      select nombre, vender_sin_stock into v_comp_name, v_comp_vender_sin_stock from public.productos where id = v_combo_line.componente_id;
      
      select id, cantidad into v_stock_id, v_stock_actual
      from public.stock_almacen
      where almacen_id = v_almacen_id and producto_id = v_combo_line.componente_id
      for update;
      ...
      if not coalesce(v_comp_vender_sin_stock, false) and v_new_qty < 0 then
        raise exception 'Stock insuficiente para el componente "%" del combo "%". Disponible: %, solicitado: %',
          coalesce(v_comp_name, v_combo_line.componente_id::text),
      ...
      if v_stock_id is not null then
        ...
        values (v_venta.empresa_id, v_almacen_id, v_combo_line.componente_id, v_new_qty);
      end if;
      ...
      values
        (gen_random_uuid(), v_venta.empresa_id, 'salida', v_combo_line.componente_id, (coalesce(new.cantidad, 0) * coalesce(v_combo_line.cantidad, 1)),
