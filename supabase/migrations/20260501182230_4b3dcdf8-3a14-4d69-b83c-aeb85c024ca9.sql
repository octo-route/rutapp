...
BEGIN
  ...
  enriched AS (
    SELECT
      b.*,
      -- dias sin actividad: si nunca uso, usar created_at; si uso, usar ultima actividad
      CASE
  ...
  SELECT
    ...
    e2.empresa_created_at::timestamptz,
    e2.status::text,
    e2.trial_ends_at::timestamptz,
    e2.fecha_vencimiento::date,
    e2.current_period_end::timestamptz,
    e2.last_sign_in_at::timestamptz,
    e2.last_venta_at::timestamptz,
  ...
  WHERE
    ...
    -- Y ADEMAS debe estar vencido O sin suscripcion activa
    AND (
