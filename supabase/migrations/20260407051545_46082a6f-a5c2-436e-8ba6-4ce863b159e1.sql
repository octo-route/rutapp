    ...
    IF v_has_active_carga THEN
      ...
      IF v_stock_id IS NOT NULL THEN
        UPDATE public.stock_camion
          SET cantidad_actual = COALESCE(v_stock_actual, 0) + COALESCE(v_linea.cantidad, 0)
          WHERE id = v_stock_id;
      ELSE
        INSERT INTO public.stock_camion
          (empresa_id, vendedor_id, producto_id, cantidad_inicial, cantidad_actual, fecha)
      ...
      VALUES
        (gen_random_uuid(), NEW.empresa_id, 'entrada', v_linea.producto_id, v_linea.cantidad,
         NEW.vendedor_id, 'cancelacion_venta', NEW.id, COALESCE(NEW.vendedor_id, NEW.cliente_id),
         COALESCE(NEW.fecha, current_date), now(),
         concat('Vuelta a borrador ', COALESCE(NEW.folio, NEW.id::text), ' (devuelto a ruta)'));
    ...
    ELSIF NEW.almacen_id IS NOT NULL THEN
      ...
      VALUES
        (gen_random_uuid(), NEW.empresa_id, 'entrada', v_linea.producto_id, v_linea.cantidad,
         ...
         COALESCE(NEW.fecha, current_date), now(),
         concat('Vuelta a borrador venta ', COALESCE(NEW.folio, NEW.id::text)));
    END IF;
  END LOOP;
