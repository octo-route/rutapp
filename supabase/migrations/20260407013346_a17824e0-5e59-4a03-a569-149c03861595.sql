...
-- 1. Update the trigger function to set vendedor_destino_id on POS sales
...
begin
  ...
  -- POS sale from almacen: also set vendedor_destino_id for vendedor kardex visibility
  insert into public.movimientos_inventario (id, empresa_id, tipo, producto_id, cantidad, almacen_origen_id, vendedor_destino_id, referencia_tipo, referencia_id, user_id, fecha, created_at, notas)
  values (gen_random_uuid(), v_venta.empresa_id, 'salida', new.producto_id, new.cantidad, v_venta.almacen_id, v_venta.vendedor_id, 'venta', v_venta.id, coalesce(v_venta.vendedor_id, v_venta.cliente_id), coalesce(v_venta.fecha, current_date), now(), concat('Venta POS ', coalesce(v_venta.folio, v_venta.id::text)));

  return new;
end;
$$;

-- 2. Backfill: set vendedor_destino_id on existing POS venta movements that have a vendedor
UPDATE movimientos_inventario mi
SET vendedor_destino_id = v.vendedor_id
FROM ventas v
WHERE mi.referencia_tipo = 'venta'
