...
BEGIN
  ...
  SELECT
    ...
    e.email::text,
    e.telefono::text,
    (SELECT u.email::text FROM auth.users u WHERE u.id = e.owner_user_id) AS owner_email,
    e.created_at::timestamptz,
    s.status::text,
    s.trial_ends_at::timestamptz,
    s.fecha_vencimiento::date,
    s.current_period_end::timestamptz,
    la.last_sign_in_at::timestamptz,
    va.last_venta_at::timestamptz,
    GREATEST(0, FLOOR(EXTRACT(EPOCH FROM (now() - COALESCE(la.last_sign_in_at, va.last_venta_at, e.created_at))) / 86400)::int) AS dias_sin_actividad,
    (CASE
    ...
    (CASE
      ...
      WHEN s.status = 'trial' AND s.trial_ends_at < (now() - (p_dias_vencido || ' days')::interval)
        THEN 'Trial expirado hace ' || FLOOR(EXTRACT(EPOCH FROM (now() - s.trial_ends_at)) / 86400)::int || ' días'
      WHEN COALESCE(la.last_sign_in_at, va.last_venta_at, e.created_at) < (now() - (p_dias_inactivo || ' days')::interval)
        THEN 'Sin actividad hace ' || FLOOR(EXTRACT(EPOCH FROM (now() - COALESCE(la.last_sign_in_at, va.last_venta_at, e.created_at))) / 86400)::int || ' días'
      ELSE 'N/A'
    END)::text AS motivo,
